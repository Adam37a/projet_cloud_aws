...
def process_records(features):
    """Extrait tous les champs du flux officiel + ajoute un ID unique."""
    records = []
    ...
    for f in features:
        ...
        record_id = f"{p.get('gid', 'unk')}_{uuid.uuid4().hex[:8]}"

        rec = {
            "id": record_id,  # <-- clé unique
            "twgid": p.get("twgid"),
            "code": p.get("code"),
            ...
            "timestamp_capture": capture_time,
            "ref_speed": ref_speed,
...
def upload_to_s3(records):
    """Envoie directement le JSON en mémoire vers S3, dans un dossier structuré."""
    date_prefix = datetime.utcnow().strftime("%Y-%m-%d")
    file_name = f"traffic_{datetime.utcnow().strftime('%H%M')}.json"
    s3_key = f"supervision_trafic_temps_reel/{date_prefix}/{file_name}"
